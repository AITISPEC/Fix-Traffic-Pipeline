...
namespace PlatformLauncher.Core.UseCases
{
    public class InstallGameUseCase
    {
        ...
        public async Task<(bool Success, string ErrorMessage, GamePreset UpdatedPreset)> ExecuteAsync(GamePreset preset, IProgress<string> progress)
        {
            try
            {
                var (success, error) = await _gameInstallService.InstallGameAsync(preset, progress);
                if (!success)
                    return (false, error, null);

                // Обновляем пресеты и возвращаем обновлённый объект
