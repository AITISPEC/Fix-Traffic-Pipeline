...
{
    /// <summary>
    /// Управление встроенным виртуальным окружением Python 3.13 — распаковывает embed-версию, пишет .pth хуки в python313._pth, устанавливает pip + зависимости через get-pip.py и wheel-файлы.
    /// Узкое место: RunProcessAsync использует Process.WaitForExitAsync() для каждого pip/python вызова; многопоточные операции (OutputDataReceived, ErrorDataReceived) идут параллельно, но блокируют UI на длительных установках.
    /// </summary>
    public class PythonEnvironmentManager : IPythonEnvironmentManager
    {
        ...
        /// <summary>Поток обработки: MainViewModel.EnsureEnvironment → EnsureEnvironmentAsync → FindSystemPython/ExtractZip → InstallPackagesAsync.</summary>
            ...
            try
            {
                ...
                else
                {
                    ...
                    if (!File.Exists(_pythonExe))
                    {
                        ...
                        {
                            // .pth-файл — встроенный механизм Python для инициализации путей; узкое место — многократное чтение/запись одного файла при каждом старте.
        ...
        /// <summary>Поток обработки: pipCheck → get-pip.py (если нужно) → foreach wheel install.</summary>
        ...
        /// <summary>Проверяет наличие системного Python 3.13 через Process.Start() — узкое место: таймаут 2000 мс фиксированный.</summary>
        private string FindSystemPython()
        {
            try
            {
                ...
                // WaitforExit(2000) — жесткий таймаут; зависший python (например, из-за прокси) блокирует продолжение.
                p.WaitForExit(2000);
        ...
        /// <summary>Поток обработки: создать psi → Start() → BeginOutputReadLine/BeginErrorReadLine (асинхронное чтение) → WaitForExitAsync().</summary>
        private async Task<int> RunProcessAsync(string fileName, string arguments, IProgress<string> progress)
        {
            ...
            if (progress != null)
            {
                // Многопоточный ввод в IProgress — не блокирует UI.
